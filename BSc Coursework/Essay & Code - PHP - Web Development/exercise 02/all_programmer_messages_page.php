<?php
	...
	// Ζητούνται από τη βάση όλα τα μηνύματα που υπάρχουν καταχωρημένα για αυτόν τον προγραμματιστή.
	$messages = get_messages_headers($_SESSION['id'], true, false);
	
	if (!$messages)
	{
		// Δεν υπάρχουν μηνυματα για τον προγραμματιστή.
	...
	while ( $row = mysql_fetch_row($messages))
	{
		...
		echo $row[0];
		print '" name="message_id"><input type="submit" value="Read"></td></form>';
		print '<form action="reply_to_manager_message_page.php" method="post"><td><input type="hidden" value="';
		echo $row[5];
